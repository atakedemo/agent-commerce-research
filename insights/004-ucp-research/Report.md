...
### Identity Linking の理解

**調査ソース**:
- `docs/specification/identity-linking.md`（`main` 先端、スナップショット 2026-06-07）
- ブランチ `remotes/origin/feat/identity-linking-v2`（最新コミット `1119e7e` — "feat!: delegated IdP, identity chaining, and capability-driven scope model"）
- ブランチ `remotes/origin/feat/identity-with-idp`（最新コミット `603f4c8` — "feat!: add delegated IdP, identity chaining, and provider discovery"）
- GitHub 比較 URL: `https://github.com/Universal-Commerce-Protocol/ucp/compare/main...feat/identity-with-idp`

---

#### 1. 目的

`dev.ucp.common.identity_linking` は、**Platform**（AI エージェント等のショッピング体験を提供するアプリ）が **Buyer**（ユーザー）の代わりに **Business**（EC サイト）で認可されたアクションを実行するための OAuth 2.0 ベースの仕組みである。

3 段階のアクセスレベルを実現する点が設計上の核心で、Identity Linking は「ゲートキーパー」ではなく**エクスペリエンスのアップグレード機構**として位置づけられている（ケイパビリティは Identity Linking の有無にかかわらず交渉から除外されない）。

| アクセスレベル | 認証主体 | 例 |
| :--- | :--- | :--- |
| **Public** | なし | 公開カタログ閲覧 |
| **Agent-authenticated** | Platform credentials（`client_id` / `client_secret`）のみ | ゲスト checkout、カート作成 |
| **Buyer-authenticated** | Platform credentials ＋ Buyer identity token | 保存住所の自動入力、注文履歴、ロイヤリティ、パーソナライズ価格 |

Identity Linking のユースケースとして明示されているものは次のとおりである: ロイヤリティ特典の利用、パーソナライズドオファー、ウィッシュリスト管理、保存済み住所の取得、注文履歴の閲覧、認証済みチェックアウト。

---

#### 2. どのステップで必要となるか

| EC ステップ | ケイパビリティ | Identity Linking の必要性 |
| :--- | :--- | :--- |
| 商品検索 | `dev.ucp.shopping.catalog.search` / `.lookup` | **不要**（Public access、認証なしで動作） |
| 商品詳細閲覧 | `dev.ucp.shopping.catalog` → `get_product` | **不要**（同上） |
| カート追加 | `dev.ucp.shopping.cart` | **不要**（Agent-authenticated で動作） |
| チェックアウト | `dev.ucp.shopping.checkout` | **任意**（`auth_required` デフォルト `false`）。ゲスト checkout は Identity Linking なしで動作。保存住所・ロイヤリティを使う場合は Buyer-authenticated が必要。Business が `auth_required: true` を設定した場合は必須 |
| 注文情報確認 | `dev.ucp.shopping.order` | **高頻度で必須**。仕様例では `auth_required: true` として宣言されており、Buyer identity token なしのリクエストに対して Business は `identity_required` エラーを返す |
| ロイヤリティ・ウィッシュリスト・保存住所 | 各拡張ケイパビリティ | **必須**（Buyer-authenticated でのみ利用可能） |

**設計上の重要点**: Business が `auth_required: false`（デフォルト）の場合、Platform は Identity Linking なしで即座に commerce フローを開始できる。Buyer が後から認証すると体験がアップグレードされる（例: 住所フォームが自動入力される）。B2B 卸売り等では全ケイパビリティが `auth_required: true` になり得る。

---

#### 3. Identity はいずれのものか（参加者の整理）

`feat/identity-with-idp` では参加者テーブルが次のように整理されている。

| UCP ロール | Identity ロール | 説明 |
| :--- | :--- | :--- |
| **Platform** | User Agent | IdP 選択・account linking・ per-Business への consent 仲介を担う。Buyer の代理として行動する |
| **Business** | Relying Party（RP） | IdP assertion を検証し、自身の authority で access token を発行する。EC サイト側 |
| **Identity Provider（IdP）** | Identity Provider | Buyer を認証し token を発行する。Business 自身でもよく、外部（Google 等）でもよい |
| **Buyer** | Resource Owner | リンク対象の本人。OAuth 同意フローで明示的に consent を付与する |

`config.providers` には、Business が信頼する IdP を宣言する（`main` ブランチには存在しない概念）。Business 自身も `auth_url` を自ドメインに向けて self-listed providers として登録できる。

```json
// /.well-known/ucp 内の identity_linking config 例（feat/identity-with-idp より）
"dev.ucp.common.identity_linking": [{
  "config": {
    "providers": {
      "app.example.login": { "auth_url": "https://accounts.example-login.app/" },
      "com.example.merchant": { "auth_url": "https://merchant.example.com/" }
    },
    "capabilities": {
      "dev.ucp.shopping.checkout": {},
      "dev.ucp.shopping.order": { "auth_required": true, "scopes": ["read", "manage"] }
    }
  }
}]
```
...
#### 4. 現行仕様（`main` ブランチ）の状態

`main` 先端の `docs/specification/identity-linking.md` は **162 行**の簡素な文書で、以下の内容に留まっている。

- OAuth 2.0 Authorization Code flow が主機構
- scope は `ucp:scopes:checkout_session` のみ（CheckoutSession の全操作を包括する 1 スコープ）
- `config.providers`（外部 IdP 宣言）は存在しない
- PKCE は **SHOULD**（オプション扱い）
- Business 向け要件のみ記述、Buyer は登場人物として明示されていない
- IdP chaining（OAuth Identity and Authorization Chaining）の概念なし
- Headless / Agentic 向け対応なし

---

#### 5. `feat/identity-linking-v2` での変更内容

`main` からのデルタとして、**大規模な再設計**が行われている（約 700 行、コミットメッセージ "feat!: delegated IdP, identity chaining, and capability-driven scope model"）。

| 変更カテゴリ | 内容 |
| :--- | :--- |
| **外部 IdP サポート**（`config.providers`） | Business が信頼する IdP を `auth_url` とともに宣言できる。Platform は business 側の advertised list から対応 IdP を選択する |
| **Accelerated IdP Flow**（IdP chaining） | 一度 IdP と account linking した Platform は、同じ IdP を信頼する別 Business に対してブラウザリダイレクトなしでトークンを取得できる。OAuth RFC 8693（Token Exchange）＋ RFC 7523（JWT bearer assertion）を組み合わせた仕組み |
| **ケイパビリティ別スコープ設定**（`config.capabilities`） | ケイパビリティごとに `required: true` と `scopes`（sub-scope）を宣言できる。スコープ導出は能力交渉後の intersection から動的に計算する |
| **Scope 命名の変更** | `ucp:scopes:checkout_session` → **capability 名を直接スコープ識別子として使用**（例: `dev.ucp.shopping.checkout`、`dev.ucp.shopping.order:read`） |
| **PKCE 必須化** | SHOULD → **MUST**（RFC 7636、`code_challenge_method=S256`） |
| **Mix-Up Attack 対策** | RFC 9207 の `iss` パラメーター検証が MUST |
| **Headless / Agentic 向け** | IdP は Device Authorization Grant（RFC 8628）を SHOULD でサポート |
| **Token Lifecycle の詳細化** | IdP token と Business-issued token の revocation は独立。Business は JWT bearer assertion grant に対して refresh token を発行すべきでない（IdP 切断後も有効なクレデンシャルが残る問題を防ぐ） |
| **IdP Requirements セクション追加** | `config.providers` に列挙する IdP が公開すべきメタデータ（`revocation_endpoint`、`jwks_uri`、`urn:ietf:params:oauth:grant-type:token-exchange`）を規定 |
| **Three Access Levels の明示** | Public / Agent-authenticated / Buyer-authenticated の 3 段階を表で定義 |

**Accelerated IdP Flow の概略（3ステップ）:**

```
1. Platform が IdP に対して Token Exchange（RFC 8693）を実施
   → grant_type: urn:ietf:params:oauth:grant-type:token-exchange
   → subject_token: 既存の IdP access token
   → resource: Business の authorization server issuer URI
   → 結果: 短命な JWT authorization grant（exp ≤ 60 秒）

2. Platform が Business の token endpoint に JWT bearer assertion（RFC 7523）を提示
   → grant_type: urn:ietf:params:oauth:grant-type:jwt-bearer
   → assertion: 上記 JWT authorization grant
   → scope: config.capabilities から導出したスコープ集合

3. Business が JWT grant を検証し、自 authority で access token を発行
   → 検証内容: iss（providers との照合）、aud（自 issuer URI）、exp、jti（replay 防止）、JWKS 署名
```

---

#### 6. `feat/identity-with-idp` での追加変更（v2 からのデルタ）

`feat/identity-linking-v2` をさらに発展させた内容（約 920 行、42 KB、コミット "feat!: add delegated IdP, identity chaining, and provider discovery"）。

| 変更カテゴリ | v2 との差分 |
| :--- | :--- |
| **参加者テーブルに Buyer を追加** | v2 の Participants 表には IdP / Business / Platform のみ。with-idp では **Buyer（Resource Owner）** を明示的に追加 |
| **フィールド名変更** | `required: true` → **`auth_required: true`**（スキーマ `source/schemas/common/identity_linking.json` も同期更新。`auth_required` が false のとき agent-authenticated として動作し buyer identity がエクスペリエンスをアップグレードする旨を明記） |
| **Discovery セクションの独立・厳格化** | RFC 8414 を primary、OIDC Discovery（`/.well-known/openid-configuration`）を 404 時の fallback とする **strict 2-tier hierarchy** を明記。404 以外のエラーでは MUST abort（MUST NOT フォールスルー） |
| **`issuer` 厳密一致の明記** | RFC 8414 metadata の `issuer` 値は byte-for-byte 一致を要求。trailing slash 除去等の正規化を行ってはならない |
| **Security Considerations セクションの追加** | PKCE（S256 のみ、plain MUST NOT）、Mix-Up Attack（RFC 9207）、`redirect_uri` exact string matching、`issuer` exactness、HTTPS 必須（TLS 1.2+）、`scopes_supported` の事前検証、token revocation、JWT grant lifetime（≤60秒）、JWT grant single-use（`jti` 追跡）、grant replay 禁止 |
| **Future Extensibility セクションの追加** | 将来バージョンで `config.mechanisms` 配列を追加予定（type discriminator による multi-mechanism negotiation — wallet attestation や verifiable credentials 等の非 OAuth 機構）。現バージョンの platform は `config` 内の未知フィールドを ignore して OAuth 2.0 にフォールバックすることを前方互換ルールとして定義 |
| **End-to-End Walkthrough の追加** | 5 ステップの具体的な HTTP リクエスト/レスポンス例（scope 導出 → discovery → authorization request → authorization response → token exchange） |
| **error_code.json への `identity_required` 追加** | `source/schemas/shopping/types/error_code.json` にエラーコードが追記 |
| **Scope 導出: 空集合のスキップ規則** | 導出スコープ集合が空の場合、Platform は identity linking フローをスキップすべき（SHOULD）旨を追加 |

**`config.capabilities` フィールド定義（with-idp の JSON Schema より）:**

| フィールド | 型 | デフォルト | 説明 |
| :--- | :--- | :--- | :--- |
| `auth_required` | boolean | `false` | `true` の場合、Buyer identity token なしのリクエストに対して Business は `identity_required` エラーを返す |
| `scopes` | string[] | （なし） | 各ケイパビリティが提供する sub-scope 識別子（例: `read`、`manage`）。存在する場合、wire format は `<capability-name>:<sub-scope>` |

---

#### まとめ：3バージョンの対比

| 観点 | `main`（現行） | `feat/identity-linking-v2` | `feat/identity-with-idp` |
| :--- | :--- | :--- | :--- |
| 参加者 | Platform、Business | Platform、Business、IdP | Platform、Business、IdP、**Buyer** |
| 外部 IdP 宣言 | なし | `config.providers` あり | 同左 |
| IdP chaining | なし | Accelerated IdP Flow（RFC 8693 + RFC 7523） | 同左（より詳細） |
| ケイパビリティ別スコープ | なし | `config.capabilities`（`required: true`） | `auth_required: true`（フィールド名変更） |
| Scope 命名 | `ucp:scopes:checkout_session` | capability 名直接（`dev.ucp.shopping.checkout`） | 同左 |
| PKCE | SHOULD | MUST | MUST（S256 のみ、plain MUST NOT） |
| Discovery | RFC 8414 のみ記述 | RFC 8414 → OIDC fallback | 厳格 2-tier hierarchy（404 のみ fallback 許可） |
| Security Considerations | なし | 一部（Token Lifecycle） | 包括的セクション |
| Headless/Agentic | なし | Device Authorization Grant（RFC 8628）SHOULD | 同左 |
| Future Extensibility | なし | なし | `config.mechanisms`（予告） |
| ファイル規模 | 162 行 | 約 700 行 | 約 920 行（42 KB） |

---

## 追加調査（2026-05-10）

参照元: [Issue #5 コメント（2026-05-10）](https://github.com/atakedemo/agent-commerce-research/issues/5#issuecomment-4411617760)

### Payment Handler の設計内容

#### UCP における Payment Handler の位置づけ

UCP は決済手段を「固定の手段リスト」ではなく、**handler 単位の仕様**として記述する。各 handler は `com.google.pay` や `dev.shopify.shop_pay` のような reverse-DNS 形式の名前を持ち、N-to-N の相互運用を可能にする。

```text
+------------------------------------------------------------------------------+
|                        Payment Handler Framework                             |
+------------------------------------------------------------------------------+
|   PARTICIPANTS → PREREQUISITES → HANDLER DECLARATION                        |
...
| 参加者         | 役割                                                               |
| :------------- | :----------------------------------------------------------------- |
| **Business**   | handler 設定を `/.well-known/ucp` に宣言し、決済 instrument を処理する |
| **Platform**   | handler を発見し、payment instrument を取得してチェックアウトへ提出する |
| **Tokenizer**  | 生の認証情報（例: カード番号）を受け取り、トークンを発行する          |
| **PSP**        | Business の代わりに実際の決済処理を行う                             |

#### Payment Handler ライフサイクル（4 フェーズ）

| フェーズ                  | 概要                                                                                              |
| :------------------------ | :------------------------------------------------------------------------------------------------ |
| **1. Prerequisites**      | 各参加者がオンボーディングを完了し `PaymentIdentity`（`access_token`）を取得する（帯域外の処理）  |
| **2. Handler Declaration**| Business が `/.well-known/ucp` の `payment_handlers` に handler 設定を宣言する                    |
| **3. Instrument Acquisition** | Platform が handler config に基づき buyer から payment instrument を取得する（checkout_id にバインド） |
| **4. Processing**         | Business（または PSP）が instrument を受け取り、binding を検証してから決済処理を行う              |

#### available_instruments の解決フロー

Platform と Business がそれぞれ独立して `available_instruments`（対応ブランド等の制約）を宣言し、**Business が両者の交差をとり `response_schema` として返す**。Platform はこれを権威ある値として扱う。

```
Platform profile:  [{ type: "card", brands: ["visa","mastercard","amex","discover"] }]
Business profile:  [{ type: "card", brands: ["visa","mastercard","amex"] }]
→ Response:        [{ type: "card", brands: ["visa","mastercard","amex"] }]  ← 権威値
```

#### Binding（バインディング）

**Binding** は payment instrument を特定の checkout セッションと Business identity に暗号的に紐付ける機構。これにより、Business A 向けに取得した credential が Business B で使い回される（リプレイ攻撃）を防ぐ。`checkout_id` と `identity` が binding の核心フィールドで、credential ペイロード内に含めることで署名対象になる。

#### Tokenization の仕組み

```text
Platform（生 credential）→ Tokenizer /tokenize → token（binding 付き）→ Business
Business（処理時）         → Tokenizer /detokenize（binding 検証）→ 生 credential → PSP
```

| トークンライフサイクル    | 説明                                      | 用途                              |
| :------------------------ | :---------------------------------------- | :-------------------------------- |
| **Single-use**            | detokenize 後に失効                        | 最もセキュア（推奨デフォルト）    |
| **TTL-based**             | 固定時間後に失効（例: 15 分）             | 一時的な失敗へのリトライ許容      |
| **Session-scoped**        | checkout セッション期間中有効             | 複数の処理試行が発生する複雑フロー |

セキュリティ要件: 128 bits 以上のエントロピー、非可逆、スコープ限定、`expiry` フィールド必須。

---

#### Shop Pay Handler（`dev.shopify.shop_pay`）の概要

| 項目                  | 内容                                                              |
| :-------------------- | :---------------------------------------------------------------- |
| **handler 名**        | `dev.shopify.shop_pay`                                            |
| **参加者**            | Merchant（Business）、Agent（Platform）、Buyer                    |
| **Business 前提条件** | Shop Pay へ登録し `shop_id` を取得（Shopify ストアは自動宣言）    |
| **Platform 前提条件** | Shop Pay へ登録し `client_id` を取得（現在はセルフサーブ未開放）  |
| **credential 型**     | `shop_token`（single-use、限定有効期間）                          |

**handler declaration 例（Business）:**
...
**Instrument Acquisition フロー（Platform 側）:**

1. Business の `payment_handlers` で `dev.shopify.shop_pay` を発見
2. checkout 合計・配送・商品・通貨・ロケールを含む payment request を構築
3. Shop Pay インターフェースを提示し、Buyer が認証・承認
4. Shop Token を受け取る
5. UCP payment instrument にラップ（`type: "shop_pay"`, `credential.type: "shop_token"`）
6. `POST /checkout-sessions/{id}/complete` へ提出

---

#### Google Pay Handler（`com.google.pay`）の概要

| 項目                  | 内容                                                              |
| :-------------------- | :---------------------------------------------------------------- |
| **handler 名**        | `com.google.pay`                                                  |
| **参加者**            | Business、Platform、PSP                                           |
| **Business 前提条件** | Google Pay に登録し `merchant_id`・`merchant_name` 等を取得       |
| **credential 型**     | PSP が復号する暗号化トークン（PAYMENT_GATEWAY トークナイズ）      |
| **instrument 型**     | `card`（brand・last_digits・billing_address 等を含む）            |
      ...
      "config": {
        ...
        "allowed_payment_methods": [{
          ...
          "tokenization_specification": {
            ...
            "parameters": { "gateway": "psp_name", "gatewayMerchantId": "merchant_001" }
          }
        }]
      }
    }]
...
1. `/.well-known/ucp` から `com.google.pay` handler を発見
2. `isReadyToPay` で Buyer のGoogle Pay利用可否を確認
3. PaymentDataRequest（merchant config・支払方法・金額・通貨）を構築
4. `loadPaymentData` で Google Pay の UI を表示・Buyer が認証
5. 返却された PaymentInstrument を `card_payment_instrument` スキーマにマッピング
6. `POST /checkout-sessions/{id}/complete` へ提出

**セキュリティ補足:** PAYMENT_GATEWAY トークナイズでは PSP のみが復号可能。DIRECT トークナイズでは Business がカードデータを直接受け取るため PCI DSS スコープが大幅に拡大する（非推奨）。

---

### 規格を元にした標準設計図（服の EC 購入ユースケース）

**前提**: UCP Payment Handler を使用したヘッドレス EC（AI エージェント / Platform が購入フローを代行）。決済は Google Pay または Shop Pay（handler は交渉で決定）。
...
sequenceDiagram
    autonumber
    actor Buyer as Buyer（購入者）
    participant Platform as Platform<br/>（AI エージェント）
    participant Business as Business<br/>（EC サイト）
    participant GPay as Google Pay<br/>（Payment Handler）
    participant PSP as PSP<br/>（決済処理）

    Note over Platform,Business: Discovery & Capability Negotiation
    Platform->>Business: GET /.well-known/ucp<br/>（UCP-Agent: platform profile）
    Business-->>Platform: UCP profile（capabilities, payment_handlers）

    Note over Buyer,Business: 1. 商品検索
    Buyer->>Platform: 「白いTシャツが欲しい」
    Platform->>Business: POST /catalog/search<br/>{ query: "白いTシャツ", filters: {category: "tops"} }
    Business-->>Platform: 検索結果（Product リスト）
    Platform-->>Buyer: 商品候補を提示

    Note over Buyer,Business: 2. 商品詳細・バリアント確認
    Buyer->>Platform: 「Mサイズの白を選ぶ」
    Platform->>Business: POST /catalog/product<br/>{ product_id: "prod_tshirt_001" }
    Business-->>Platform: Product 詳細（variants, price, inventory_signal）
    Platform-->>Buyer: バリアント・価格を確認提示

    Note over Buyer,Business: 3. カート追加
    Buyer->>Platform: 「これをカートに入れて」
    Platform->>Business: POST /carts<br/>{ line_items: [{variant_id, quantity: 1}] }
    Business-->>Platform: Cart（cart_id, totals）

    Note over Buyer,Business: 4. チェックアウト開始
    Buyer->>Platform: 「購入する」
    Platform->>Business: POST /checkout-sessions<br/>{ cart_id, UCP-Agent: platform_profile }
    Business-->>Platform: Checkout（checkout_id, status: "incomplete",<br/>payment_handlers: [com.google.pay config], totals）

    Note over Platform,GPay: 5. Payment Instrument 取得
    Platform->>GPay: isReadyToPay（allowed_payment_methods）
    GPay-->>Platform: ready: true
    Platform->>GPay: loadPaymentData（PaymentDataRequest:<br/>merchant_info, amount, currency）
    Buyer->>GPay: Google Pay UI で認証・承認
    GPay-->>Platform: PaymentData（暗号化トークン, billing_address, card display）

    Note over Platform,Business: 6. チェックアウト完了（決済）
    Platform->>Business: POST /checkout-sessions/{id}/complete<br/>{ payment: { instruments: [{ handler_id, type:"card",<br/>credential: { type:"PAYMENT_GATEWAY", token:"..." },<br/>binding: { checkout_id } }] } }
    Business->>Business: handler_id 検証、binding 確認
    Business->>PSP: 決済処理（token, amount, currency）
    PSP-->>Business: 決済結果（success）
    Business-->>Platform: Checkout（status: "completed", order_id）

    Note over Buyer,Business: 7. 注文確認
    Platform->>Business: GET /orders/{order_id}
    Business-->>Platform: Order（line_items, fulfillment, totals, status）
    Platform-->>Buyer: 注文完了・詳細を提示
```
...
erDiagram
    ...
    PLATFORM {
        ...
        string name
        string ucp_profile_url
    }

    BUSINESS {
        string business_id PK
        string name
        string ucp_well_known_url
    }

    PAYMENT_HANDLER {
        string handler_name PK
        string version
        string spec_url
        string schema_url
    }

    BUSINESS_HANDLER_CONFIG {
        string config_id PK
        string business_id FK
        string handler_name FK
        string environment
        json config_json
    }
    ...
    PLATFORM ||--o{ CART : "manages"
    PLATFORM ||--o{ CHECKOUT_SESSION : "initiates"
    BUSINESS ||--o{ PRODUCT : "sells"
    BUSINESS ||--o{ BUSINESS_HANDLER_CONFIG : "configures"
    BUSINESS ||--o{ CART : "hosts"
    BUSINESS ||--o{ CHECKOUT_SESSION : "processes"
    BUSINESS ||--o{ ORDER : "fulfills"
    PAYMENT_HANDLER ||--o{ BUSINESS_HANDLER_CONFIG : "used_by"
    PRODUCT ||--o{ PRODUCT_VARIANT : "has"
    CART ||--o{ CART_ITEM : "contains"
    CART_ITEM }o--|| PRODUCT_VARIANT : "references"
    CART ||--o| CHECKOUT_SESSION : "converted_to"
    CHECKOUT_SESSION ||--o{ PAYMENT_INSTRUMENT : "has"
    CHECKOUT_SESSION ||--o| ORDER : "becomes"
...
stateDiagram-v2
    ...
    state "Cart（カート）" as CART_STATE {
        [*] --> cart_active : POST /carts（create_cart）
        cart_active --> cart_active : PUT /carts/{id}（update_cart: 商品追加・変更）
        cart_active --> cart_canceled : POST /carts/{id}/cancel
        cart_canceled --> [*]
    }

    cart_active --> checkout_incomplete : POST /checkout-sessions（create_checkout）

    state "Checkout Session（チェックアウト）" as CHECKOUT_STATE {
        [*] --> checkout_incomplete
        checkout_incomplete --> checkout_requires_escalation : 必須情報不足・Buyer handoff 等
        checkout_requires_escalation --> checkout_incomplete : recoverable を解消（PUT update）
        checkout_incomplete --> checkout_ready : 必須情報充足
        checkout_requires_escalation --> checkout_ready : continue_url 等で充足
        checkout_ready --> checkout_in_progress : POST .../complete（instrument 提出）
        checkout_in_progress --> checkout_completed : 決済成功
        checkout_in_progress --> checkout_requires_escalation : 決済失敗（recoverable）
        checkout_incomplete --> checkout_canceled : POST .../cancel
        checkout_requires_escalation --> checkout_canceled : POST .../cancel
        checkout_ready --> checkout_canceled : POST .../cancel
    }

    state "Payment Instrument（決済）" as PAYMENT_STATE {
        [*] --> instrument_acquiring : Platform が Google Pay / Shop Pay を呼び出し
        instrument_acquiring --> instrument_acquired : Buyer 認証・token 取得
        instrument_acquiring --> instrument_failed : Buyer キャンセル / エラー
        instrument_acquired --> instrument_processing : Business が PSP へ決済処理依頼
        instrument_processing --> instrument_succeeded : PSP 承認
        instrument_processing --> instrument_declined : PSP 拒否
        instrument_failed --> [*]
        instrument_declined --> [*]
    }

    checkout_ready --> instrument_acquiring : Instrument Acquisition 開始
    instrument_acquired --> checkout_in_progress : complete_checkout へ instrument 提出

    state "Order（注文）" as ORDER_STATE {
        [*] --> order_created : Checkout completed → Order 生成
        order_created --> order_confirmed : Business が注文確認
        order_confirmed --> order_shipped : 発送完了
        order_shipped --> order_delivered : 配達完了
        order_confirmed --> order_canceled : キャンセル要求
        order_created --> order_canceled : キャンセル要求
    }

    checkout_completed --> order_created : GET /orders/{id}
    instrument_succeeded --> checkout_completed
```

---

### Payment Handler 設計のまとめ

| 観点                      | Shop Pay（`dev.shopify.shop_pay`）                        | Google Pay（`com.google.pay`）                           |
| :------------------------ | :-------------------------------------------------------- | :------------------------------------------------------- |
| **Instrument 型**         | `shop_pay`（専用型）                                      | `card`（UCP 標準）                                        |
| **Credential 型**         | `shop_token`（single-use）                                | PAYMENT_GATEWAY 暗号化トークン（PSP が復号）             |
| **Platform 前提条件**     | `client_id` 登録必要（現在セルフサーブ未開放）            | Google Pay SDK 統合のみ（事前登録不要）                  |
| **Buyer 体験**            | Shop Pay UI で認証（保存カード・住所が自動補完）          | Google Pay UI で認証（Google アカウント連携）            |
...
- Payment Handler は N-to-N の相互運用を可能にする枠組みで、`PARTICIPANTS → PREREQUISITES → HANDLER_DECLARATION → INSTRUMENT_ACQUISITION → PROCESSING` の 4 フェーズで構成される（`payment-handler-guide.md`）。
- `available_instruments` の解決は Business が権威を持ち、Platform profile と Business profile の交差を `response_schema` として返す。Platform はこれを権威値として扱わなければならない（`payment-handler-guide.md`）。
- Tokenization は `POST /tokenize`（raw credential → token）と `POST /detokenize`（binding 検証後に token → raw credential）の 2 エンドポイントで成立し、binding（`checkout_id` + `identity`）によりリプレイ攻撃を防ぐ（`tokenization-guide.md`）。
