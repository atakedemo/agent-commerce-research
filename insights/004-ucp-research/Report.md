...
UCP では **カート**と**チェックアウト**で状態の扱いが分かれる（`docs/specification/cart.md`）。カートは購入意思が固まる前のバスケット構築用で、**支払いハンドラや checkout と同様のステータス機械は持たない**（存在／`not_found` などの運用に近い）。チェックアウトは **`status` フィールドでフェーズが明示**され、プラットフォームはその値と `messages` に従い `update_checkout`・`continue_url`・`complete_checkout` を選択する（`docs/specification/checkout.md`「Checkout Status Lifecycle」）。
...
**商品検索の場面**において、Gemini や ChatGPT のような **汎用対話エージェント**側が担うことと、**各 EC サイト・小売（マーチャント）**が UCP として担うことを、次表で対比する。エージェントは UCP 用語では主に **platform**（消費者に接する側）、小売は **business**（カタログと API の権威ある提供者）に相当する（`docs/specification/overview.md`、`catalog/index.md`）。

| 観点 | 汎用対話エージェント側（platform 側の実装・運用） | 各 EC・小売側（business が UCP で公開・実装するもの） |
|------|---------------------------------------------------|--------------------------------------------------------|
| 対話・ニーズ整理 | ユーザーの自然言語を解釈し、商品探索の目的・条件を会話で補完する（UCP の外側の UX）。 | 会話 UI は持たない。検索は **HTTP／MCP 等の API** として受け付ける。 |
| どの店のカタログか | どのマーチャントの `endpoint` を呼ぶかのルーティング（ディープリンク、検索ツールの選択、マルチ店舗の整理）は **エージェント／ホストアプリの責務**。 | **`/.well-known/ucp`** に shopping service・`dev.ucp.shopping.catalog.*`・REST/MCP の `schema`／`endpoint` を掲載し、自社カタログへの入り口を宣言する。 |
| capability 交渉への参加 | **Platform profile** を用意し、リクエスト（例: `UCP-Agent`）で提示。交渉後に残った capability に従い `search`／`lookup` を呼ぶ。 | **Business profile** で提供能力を宣言し、**server-selects** に基づき実際に有効な能力集合を決める。 |
| 検索リクエストの中身 | `query`、`filters`、`pagination`、**`context`**（地域・通貨ヒント等）、**`intent`**（検索意図のテキスト）などを **JSON 要求へ写像**し、クライアントとして送信（署名・認証ヘッダを付与する場合あり）。 | **`POST /catalog/search`** または MCP の `search_catalog` 等を**実装**し、自社商品マスタに基づき UCP スキーマに沿った `products` を返す。 |
| 詳細・絞り込み | ユーザーが関心を示した SKU について **`lookup_catalog`／`get_product`**（または対応する REST）を呼び、説明や比較の材料にする。 | **バリアント ID・価格・可用性シグナル**等を返す。返却 ID は **checkout の `line_items` と整合**させる。 |
| 結果の見せ方・説明 | 一覧の要約、比較、次の質問など **LLM によるプレゼンテーション**（カタログ JSON の上乗せ）。 | カタログ JSON の**構造とフィールド意味**までを仕様で規定。会話文の生成は規定しない。 |
| 確定価格・購入可否 | カタログ表示を**参考**とし、購入時は checkout へ進めるよう導線を作る。セッション跨ぎの再利用は**再検証**が推奨される利用者側の責務に近い。 | カタログ応答は **取引上の確約ではない**。**チェックアウトが価格・エリジビリティの権威**（`catalog/index.md`「Relationship to Checkout」）。規制・在庫の最終判断は checkout 時のデータで行う。 |

- Gemini や ChatGPT のような **汎用対話エージェント**が購買を支援する場合、UCP は **「検索という行為そのもの」を会話プロトコルとして定義する**のではなく、**マーチャントが公開した Catalog capability（search／lookup）を、交渉済みトランスポート（主に REST または MCP）で呼び出す**前提に立つ。
- プラットフォームは `context`（地域・通貨ヒント）や `intent`（検索意図の自然文）などをリクエストに載せ、**関連性・パーソナライズのヒント**としてビジネスに渡すことができるが、**コンテキストは暫定的**であり、**エリジビリティや規制の最終判断はチェックアウト時の束縛データで行う**ことが明示される（`docs/specification/catalog/index.md`「Relationship to Checkout」「Context」）。
- Catalog で規定される範囲の要約は次のとおりである。
  - **規定するもの**: 商品・バリアント・価格・在庫シグナル等の**発見用データモデル**、全文検索・フィルタ・ページネーション（search）、ID／バッチ取得と単品詳細（lookup／get product）、**チェックアウトの `line_items` で使う ID との整合**。
  - **規定しない／弱いコミットであるもの**: カタログ応答は**取引上の確約ではなく**、**チェックアウトが権威**である。セッションをまたいだ結果の無検証再利用は推奨されない。
  - **汎用エージェントに効く設計**: マーチャント横断で同じ **Catalog API の形**を前提にできるため、エージェント実装は「自然言語 → `search_catalog`／`POST /catalog/search` の引数生成」に集約しやすい。一方、**会話専用の必須スキーマ**は Checkout の A2A binding 側に寄せられており、**「検索＝REST/MCP の Catalog、購入確定＝Checkout（＋任意で A2A）」**という役割分担になる。

公式の Catalog 文書索引: [Universal-Commerce-Protocol/ucp の `docs/specification/catalog`](https://github.com/Universal-Commerce-Protocol/ucp/tree/main/docs/specification/catalog)。
...
- 商品検索では platform（汎用エージェント等）が Catalog API のクライアント・対話・`context`/`intent` の写像を担い、business（小売）が `/.well-known/ucp` と search／lookup のサーバ実装および ID 整合を担う。価格・購入可否の確定はチェックアウトが権威（`catalog/index.md`）。
