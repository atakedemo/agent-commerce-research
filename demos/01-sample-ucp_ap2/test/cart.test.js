 ...
 *
 * Status: TODO — b-mcp-server does not yet implement cart tools.
...
const SKIP_REASON = "b-mcp-server does not implement cart tools yet";

// ─── create_cart (§6) ────────────────────────────────────────────────────────

describe("create_cart (§6)", () => {
  it(
    "creates a cart and returns id, line_items, totals, continue_url",
    { skip: SKIP_REASON },
    async () => {
  ...
  it(
    "creates a cart with no line_items (empty cart)",
    { skip: SKIP_REASON },
    async () => {
      // const { isError, data } = await callTool(client, "create_cart", {
      //   meta: META,
...
describe("get_cart (§7)", () => {
  ...
  it(
    "returns error for unknown cart id",
    { skip: SKIP_REASON },
    async () => {
...
describe("update_cart (§8)", () => {
  it(
    "replaces entire cart with new line_items state",
    { skip: SKIP_REASON },
    async () => {
      // const { data: created } = await callTool(client, "create_cart", {
      //   meta: META,
  ...
  it(
    "adding items follows get → merge → update pattern",
    { skip: SKIP_REASON },
    async () => {
      // const { data: created } = await callTool(client, "create_cart", {
      //   meta: META,
...
describe("cancel_cart (§8)", () => {
  ...
  it(
    "canceled cart cannot be updated",
    { skip: SKIP_REASON },
    async () => {
      // const { data: created } = await callTool(client, "create_cart", {
      //   meta: META,
